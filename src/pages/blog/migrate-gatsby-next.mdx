...
Turns out there is a solution for this -- I'm not the only one still using RSS. There are a few helper packages for extracting frontmatter from MDX and making it easier to work with. I chose to use [`next-mdx-enhanced`](https://github.com/hashicorp/next-mdx-enhanced). Others ([`next-mdx-remote`](https://github.com/hashicorp/next-mdx-remote), [`mdx-bundler`](https://github.com/kentcdodds/mdx-bundler)) are faster, offer more flexibility, etc., but I wanted something that let me keep the blog posts in the `pages` directory. There were tradeoffs to this, of course, but using frontmatter meant I could use [`gray-matter`](https://github.com/jonschlinkert/gray-matter) to extract the info needed to generate the RSS feed.
