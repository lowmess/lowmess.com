...
That's enough for a basic toggle, but now loading in new pages or refreshing will always revert to the default system color theme. To fix that, we'll want to store the result of each change in `localStorage` (or even as a database column if you are working on a project that stores user preferences in a table).
...
We might as well also set the default value on our `<select>` so the correct option is checked:
